...
@Composable
fun WeatherHourlyForecast(modifier: Modifier = Modifier, hourly: Hourly?) {
    val state = rememberLazyListState()

    val itemSize =
        hourly?.time?.size ?: hourly?.weatherCode?.size ?: hourly?.temperature2m?.size ?: 0
    ...
    CompositionLocalProvider(LocalOverscrollConfiguration provides null) {
        LazyRow(
            ...
            horizontalArrangement =
                if (itemSize != 0) Arrangement.spacedBy(MaterialTheme.spacing.small)
            ...
            if (itemSize != 0) {
                ...
                items(itemCount) { index ->
                    WeatherHourItem(
                        date =
                            hourly?.time?.get(currentHourIndex + index)?.let {
                                getDateFromDateAndTime(it)
                            },
            ...
            WeatherHourlyForecast(
                hourly =
                    Hourly(
                        ...
                        temperature2m =
                            listOf(30.0, 30.0, 30.0, 30.0, 30.0, 30.0, 30.0, 30.0, 30.0, 30.0),
                    )
            )
        }
    }
